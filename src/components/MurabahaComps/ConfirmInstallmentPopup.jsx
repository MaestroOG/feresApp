...
const ConfirmInstallmentPopup = ({ onCancel, onConfirm }) => {
    return (
        <div className='fixed h-screen w-full bg-[#06060640] top-0 left-0'>
            <div className='bg-white fixed w-full min-h-[274px] rounded-t-[20px] bottom-0 left-0'>
                ...
                <div className='w-full flex items-center justify-between px-4 mt-5'>
                    <button className='w-[48%] bg-[#F8F8F8] rounded-[30px] px-4 py-[18px] text-lg text-[#2F2F3F] font-semibold' onClick={onCancel}>No, don't</button>
                    <button className='w-[48%] bg-[#0AB247] rounded-[30px] px-4 py-[18px] text-lg text-white font-semibold' onClick={onConfirm}>Yes, confirm</button>
                </div>
            </div>
        </div>
    )
